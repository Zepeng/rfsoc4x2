...
1. Create a Vitis Platform project:
 ...
 - Once in the `xsct` terminal, execute the following commands to create a Vitis platform project:
   ...
   If you previously built the single-stream version or a platform without RFDC clock IDs `3` and `4`, create a fresh platform project or delete the old `~/workspace/rfsoc_adc_vitis_platform` first. This avoids Vitis using cached stream or clock metadata. After exporting a new `.xsa`, the Vitis platform must be regenerated before the application project is configured or rebuilt.
...
3. Build the Vitis platform:  
 - Open the Vitis GUI with the same workspace used by `xsct`:
   ...
   ```
 - Import the generated platform project into the Vitis workspace:
   - Select **<em>File->Import...</em>**.
   - Select **<em>General->Existing Projects into Workspace</em>** and click **<em>Next</em>**.
   - Set **<em>Select root directory</em>** to `~/workspace`.
   - Enable **<em>Search for nested projects</em>** if that option is shown.
   - Select `rfsoc_adc_vitis_platform`.
   - Leave **<em>Copy projects into workspace</em>** unchecked because the project is already under `~/workspace`.
   - Click **<em>Finish</em>**.

   If the GUI import does not find the platform, close Vitis and use a standalone `xsct` process launched from a Linux terminal. Do not use the SDK XSCT console embedded in the IDE:
   ```shell
   cd ~/workspace
   xsct
   ```
   ...
   ```tcl
   setws ~/workspace
   importprojects ~/workspace/rfsoc_adc_vitis_platform
   exit
   ```
   Restart Vitis with `vitis -workspace ~/workspace`. The imported platform project should now appear in the **<em>Explorer</em>** window.
 ...
 - Verify that the exported platform exposes RFDC tile 0 clock ID `3` as a fixed `307.2 MHz` clock before creating the application:
   ```shell
   platforminfo -v -p \
     ~/workspace/rfsoc_adc_vitis_platform/export/rfsoc_adc_vitis_platform/rfsoc_adc_vitis_platform.xpfm | \
     sed -n '/Clock Information/,/Memory Information/p'
   ```
   The clock list must include clock ID `3` with status `fixed` and frequency `307200000`. Do not continue to the application build if clock ID `3` is missing or reported as a scaled clock.
...
2. Replace the template sources with the two-stream sources:
   ...
   - The kernel arguments are now:
     ...
     - `size`: number of 128-bit AXI4-Stream words to capture from each stream. The current kernel requires `8192`.
     ...
     The kernel continuously writes both streams into one full-frame circular UltraRAM buffer through one uninterrupted `II=1` acquisition loop. After the ADC_C threshold crossing and the 80% post-trigger interval, it freezes the ring and copies the chronological 8192-word waveform to DDR as packed 256-bit words.
...
7. Run the `test_adc` app to grab samples from the ADC:
   ...
   ```
   `wave.txt` and the Ethernet stream contain two columns/channels: `RFDC_DATA_AXIS` followed by `RFDC_TRIG_AXIS`. Each frame contains about 20% pretrigger and 80% post-trigger samples; for the default frame size, the trigger word is near sample `13104`.

   If the program stops after `Waiting for trigger for frame 0`, XRT has programmed the PL and launched the compute unit, but the HLS kernel is probably waiting for ADC_C/RFDC_TRIG_AXIS to cross the configured threshold. Lower `--threshold`, confirm the signal on ADC_C, and confirm both RFDC streams are running. Recheck the reference clock setup above and inspect the XRT logs:
   ```shell
