...
Run it from a shell batch session with no Vivado project already open. If a
project is open, the checker inspects that project instead of recreating the
design; an older project can therefore report the removed fifth PPS ILA probe.
...
The checked-in kernel maps the 10-us model window onto 3072 two-sample RFDC
words at 614.4 MS/s. The window starts at the accepted PPS word, which is word
1638 of the chronological 8192-word capture. For each of the 250 selected model
bins, the generated map chooses the RFDC word nearest that bin's center. It
reads two selected features per cycle from the dual-port average BRAM and calls
the generated Conifer `bdt.decision_function()` before starting waveform DDR
writeout. Identical capture buffers therefore use identical BDT source words
regardless of kernel launch time.

The BDT waveform remains the two-sample time average of the signed
four-channel sum `ADC_D + ADC_C + ADC_B + ADC_A`. The runtime sum gate uses the
same four-channel sum but still evaluates each full-rate sample rather than
the time average.

The real-BDT HLS configuration defines `BDT_USE_NORM_CONFIG` and uses the
global training z-score in `src/vitis_adc_platform/bdt_norm_config.h`.
The checked-in values are reconstructed from the integer-count lattice in the
deployed, normalized `pynq_data/X_test.npy`: mean `-0.0897942781` and inverse
sigma `1.54739702`. Event acceptance remains controlled only by PPS and the
runtime sum mode; the early BDT score remains diagnostic and is packed into the
existing metadata word for the host.

For a meaningful bench comparison, drive a 10-us waveform with PPS marking
its first sample. The signed four-channel RFSoC sum must also have the same
polarity and ADC-count scale as the waveform used for BDT training. A
normalized arbitrary waveform with an unrelated Vpp does not reproduce the
model input, even when its shape and PPS timing are repeatable.
...
```bash
TIMING=build/vitis_dummy_kernel_2025_2/link_bdt_word2/reports/link/imp/\
impl_1_system_wrapper_timing_summary_routed.rpt
grep -nE 'Design Timing Summary|WNS\(ns\)|Timing constraints are' "$TIMING"

xclbinutil --info \
  --input build/vitis_dummy_kernel_2025_2/dummy_kernel_bdt_word2.xclbin
```
...
file build/vitis_dummy_kernel_2025_2/test_adc_word2
test -x build/vitis_dummy_kernel_2025_2/test_adc_word2
```

`file` must report an ARM AArch64 executable. Package the word2 link result,
matching host, and existing verified PetaLinux images. `--package.no_image`
creates the deployable `sd_card` directory without rebuilding a full SD image:

```bash
export RFSOC4X2_LINUX_IMAGE_DIR="$PWD/build/rfsoc-linux-2025_2/images/linux"
export RFSOC4X2_WORD2_PACKAGE="$PWD/build/vitis_dummy_kernel_2025_2/package_bdt_word2"

v++ --package --target hw \
  --platform "$RFSOC4X2_XPFM" \
  --package.out_dir "$RFSOC4X2_WORD2_PACKAGE" \
  --package.boot_mode=sd \
  --package.no_image \
  --package.image_format=ext4 \
  --package.rootfs "$RFSOC4X2_LINUX_IMAGE_DIR/rootfs.ext4" \
  --package.kernel_image "$RFSOC4X2_LINUX_IMAGE_DIR/Image" \
  --package.dtb "$RFSOC4X2_LINUX_IMAGE_DIR/system.dtb" \
  --package.sd_file "$PWD/build/vitis_dummy_kernel_2025_2/test_adc_word2" \
  --package.sd_file "$PWD/build/vitis_dummy_kernel_2025_2/dummy_kernel_bdt_word2.xclbin" \
  "$PWD/build/vitis_dummy_kernel_2025_2/dummy_kernel_bdt_word2.xclbin"

for f in BOOT.BIN boot.scr Image system.dtb \
         test_adc_word2 dummy_kernel_bdt_word2.xclbin; do
  test -s "$RFSOC4X2_WORD2_PACKAGE/sd_card/$f" ||
    echo "MISSING: $f"
done
```

No output from the loop means the package is complete. Power the RFSoC board
off before replacing its boot files. Copy the complete package directory to
the FAT32 boot partition; leave the existing rootfs partition unchanged:

```bash
sudo mkdir -p /mnt/rfsoc4x2-boot
sudo mount /dev/sda1 /mnt/rfsoc4x2-boot
sudo rsync -rt --no-owner --no-group --omit-dir-times \
  "$RFSOC4X2_WORD2_PACKAGE/sd_card/" /mnt/rfsoc4x2-boot/

sudo cmp "$RFSOC4X2_WORD2_PACKAGE/sd_card/BOOT.BIN" \
  /mnt/rfsoc4x2-boot/BOOT.BIN
sudo cmp "$RFSOC4X2_WORD2_PACKAGE/sd_card/dummy_kernel_bdt_word2.xclbin" \
  /mnt/rfsoc4x2-boot/dummy_kernel_bdt_word2.xclbin
sudo cmp "$RFSOC4X2_WORD2_PACKAGE/sd_card/test_adc_word2" \
  /mnt/rfsoc4x2-boot/test_adc_word2
